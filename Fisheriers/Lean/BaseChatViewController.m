...
- (void)scrollToBottom
{
    ...
    if (self.messageTableView.contentSize.height > self.messageTableView.bounds.size.height) {
        yOffset = self.messageTableView.contentSize.height - self.messageTableView.bounds.size.height;
    }
    // 设置偏移量为最底部
    [self.messageTableView setContentOffset:CGPointMake(0, yOffset) animated:NO];
}
